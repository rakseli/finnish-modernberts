...
WORD_RE = re.compile(r'\b(?:[A-ÅÄÖÀØÈČŠÆŊŦŋŽ][a-zàøèčšåäöđŧž]{1,}|[a-zàøèčšåäöđŧž]{2,})\b')

# Short word regex for --short-ratio option
SHORT_WORD_RE = re.compile(r'\b[a-zA-ZàøèčšåäöđŧžÅÄÖÀØÈČŠÆŊŦŋŽ]\b')

# Tokenization regex for --type-token-ratio option
TOKENIZE_RE = re.compile(r'([^\W\d_]+|\s+|.)')

FOREIGN_LETTER = re.compile(r'[^\.\n\W\d_a-zA-ZàøèčšåäöđŧžÅÄÖÀØÈČŠÆŊŦŋŽ\u200B-\u200F\uFEFF\u2060\u2063\u2064]')
...
def argparser():
    ...
    ap.add_argument('--nlfcl',action='store_true')
    ap.add_argument('--jsonl', nargs='+')
    return ap

def get_meta(json_field,value,meta,urls=False):
    if value in str(meta[json_field]):
        v = str(json_field+":"+value)
    else:
        v = None
    return v
...
def filter_meta(stats,json_field,value,jsonl_row,args):
    filter_condition = get_meta(json_field,value,jsonl_row)
    if filter_condition is None:
        result = 'pass-all'
        skip = False
    else:
...
def filter_jsonl(fn, args):
    ...
    with open(fn) as f:
        for ln, line in enumerate(f, start=1):
            ...
            stats['total-docs'] += 1
            if args.nlfcl:
                meta = data['meta']['sourcemeta']
                if not filter_meta(stats,'contributor','Lönnrot',meta,args):
                    try:
                        print(line, end='')
